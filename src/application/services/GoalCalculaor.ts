...
export class GoalCalculator {
  ...
  private static activityMultipliers: Record<Profile.ActivityLevel, number> = {
    ...
    [Profile.ActivityLevel.ATHETE]: 1.9,
  };
  ...
  static calculate(profile: Profile): GoalCalculator.CalculateGoalResult {
    ...
    const porteinCalories = proteinGrams * 4;

    const fatGrams = Math.round(weight * fats);
    const fatCaliries = fatGrams * 9;

    const remainingCalories = calories - porteinCalories - fatCaliries;

    const carbsGrams = Math.round(remainingCalories / 4);
    const carbsCalories = carbsGrams * 4;

    const realCalories = porteinCalories + fatCaliries + carbsCalories;
  ...
  private static calcCalories({
    ...
    const bmr = gender === Profile.Gender.MALE
