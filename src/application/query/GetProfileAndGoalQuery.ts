...
export class GetProfileAndGoalQuery {
  ...
  async execute({
    ...
    const { Items = [] } = await dynamoCient.send(command);

    const profile = Items.find((item): item is GetProfileAndGoalQuery.ProfileItemType => item.type === ProfileItem.type);
    const goal = Items.find((item): item is GetProfileAndGoalQuery.GoalItemType => item.type === GoalItem.type);
